...
(function() {
  $(function() {
    ...
    $("form.new_product").on("ajax:success", function(event, data, status, xhr) {
      $("form.new_product")[0].reset();
      $('#newproduct_modal').modal('hide');
      return $('#error_explanation').hide();
    });
    $("form.new_product").on("ajax:error", function(event, xhr, status, error) {
      var errors;
      errors = jQuery.parseJSON(xhr.responseText);
      $('#error_explanation').empty();
      $('#error_explanation').append('<ul>');
      $.each(errors, function(field, error) {
        return $('#error_explanation').append('<li>' + field.capitalize() + ' ' + error[0] + '</li>');
      });
      $('#error_explanation').append('</ul>');
      return $('#error_explanation').show();
    });
    return $('.like').bind('ajax:success', function(event, data, status, xhr) {
      var id, votes;
      id = data.id;
